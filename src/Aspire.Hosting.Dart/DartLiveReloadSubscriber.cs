...
/// <summary>
/// Restarts every Dart application that asked for live reload when its source files change.
/// </summary>
/// <remarks>
...
/// </remarks>
internal sealed class DartLiveReloadSubscriber(
...
{
    private readonly ConcurrentDictionary<string, DartLiveReloadWatcher> _watchers = new();
    ...
    {
        foreach (var resource in @event.Model.Resources.OfType<DartAppResource>())
        {
            if (!resource.HasAnnotationOfType<DartLiveReloadAnnotation>())
            {
                continue;
            }
    ...
    private void StartWatcher(DartAppResource resource)
    {
        ...
        var watcher = new DartLiveReloadWatcher(
    ...
    private async Task RestartAsync(DartAppResource resource, string changedFile, CancellationToken cancellationToken)
    {
