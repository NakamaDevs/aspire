...
# This script validates the Elixir AppHost SDK with Redis integration
set -e

echo "=== Elixir AppHost SDK Validation ==="
...
# Initialize Elixir AppHost
echo "Creating Elixir apphost project..."
aspire init --language elixir --non-interactive -d
...
# Insert Redis code into apphost.exs
echo "Configuring apphost.exs with Redis..."
if [ -f "apphost.exs" ] && grep -q "^builder = Aspire.create_builder!()$" apphost.exs; then
    awk '
    /^builder = Aspire\.create_builder!\(\)$/ {
        print
        print ""
        print "# Add Redis cache resource"
        print "cache = Aspire.DistributedApplicationBuilder.add_redis!(builder, \"cache\")"
        print "Aspire.Redis.RedisResource.with_image_registry!(cache, \"netaspireci.azurecr.io\")"
        next
    }
    { print }
    ' apphost.exs > apphost.exs.tmp && mv apphost.exs.tmp apphost.exs
    echo "✅ Redis configuration added to apphost.exs"
fi

echo "=== apphost.exs ==="
[ -f "apphost.exs" ] && cat apphost.exs
