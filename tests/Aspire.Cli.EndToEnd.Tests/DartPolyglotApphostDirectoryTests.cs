...
/// must report its real path over the backchannel. <c>AtsDartCodeGenerator</c> forwards
/// <c>ASPIRE_APPHOST_FILEPATH</c> and <c>ASPIRE_PROJECT_DIRECTORY</c> through <c>createBuilder</c>.
...
/// </summary>
public sealed class DartPolyglotApphostDirectoryTests(ITestOutputHelper output)
{
    ...
    public async Task StopDartPolyglotAppHostUsingApphostDirectory()
    {
        ...
        // AppHost again with `--apphost dartapp`. That forces discovery through the path the running
        // host reported over the backchannel instead of through the working directory.
        await auto.TypeAsync("mkdir dartapp && cd dartapp");
        ...
        var projectRoot = Path.Combine(workspace.WorkspaceRoot.FullName, "dartapp");
        ...
        await auto.TypeAsync("aspire stop --non-interactive --apphost dartapp");
        await auto.EnterAsync();
