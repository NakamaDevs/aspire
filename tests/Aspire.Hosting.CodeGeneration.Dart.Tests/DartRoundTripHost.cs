...
/// <summary>
/// Runs one round trip: a real RemoteHost JSON-RPC server in this process, the generated Dart SDK
/// in a temporary directory, and an <c>apphost.dart</c> script that the <c>dart</c> executable runs
/// against that server.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// The script waits with <c>stdin.first</c>, not with a synchronous read. A synchronous read blocks
/// the Dart event loop, and the transport then cannot answer an <c>invokeCallback</c> request that
/// the host sends while the test inspects the model.
/// </para>
/// </remarks>
internal sealed class DartRoundTripHost : IAsyncDisposable
{
    ...
    /// <summary>The directory below the AppHost that holds the generated SDK.</summary>
    internal const string ModulesDirectory = ".aspire/modules";
    ...
    /// <c>/tmp</c>.
    /// </summary>
    public static async Task<DartRoundTripHost> StartAsync(ITestOutputHelper outputHelper, CancellationToken cancellationToken = default)
    {
        var socketPath = $"/tmp/aspire-dart-{Guid.NewGuid():N}.sock";
        ...
        var roundTripHost = new DartRoundTripHost(host, socketPath, authToken, outputHelper, handleRegistries, cancellationTokenRegistries);
    ...
    /// <summary>
    /// Generates the Dart SDK for the real <c>Aspire.Hosting</c> assembly plus the shared ATS test
    /// types, writes every file into <c>.aspire/modules</c> below <paramref name="directory"/>, and
    /// writes the package manifest that <c>dart run</c> needs.
    /// </summary>
    /// <remarks>
    /// The manifest declares no dependency, so <c>dart pub get</c> needs no network access.
    /// </remarks>
    public static async Task GenerateSdkAsync(string directory, CancellationToken cancellationToken = default)
    {
        ...
        var files = new AtsDartCodeGenerator().GenerateDistributedApplication(context);

        var modules = Path.Combine(directory, ".aspire", "modules");
        Directory.CreateDirectory(modules);

        foreach (var (name, content) in files)
        {
            await File.WriteAllTextAsync(Path.Combine(modules, name), content, cancellationToken);
        }
    ...
    /// <summary>
    /// Runs <c>dart pub get</c>, which writes <c>.dart_tool/package_config.json</c>. Every later
    /// <c>dart run</c> in the same directory then starts without a resolution step.
    /// </summary>
    public static async Task RestoreAsync(string directory, ITestOutputHelper outputHelper, CancellationToken cancellationToken = default)
    {
        ...
        startInfo.ArgumentList.Add("pub");
        ...
        {
            throw new InvalidOperationException($"`dart pub get` exited with code {process.ExitCode}.\n{output}");
        }
    }
    ...
    /// <summary>
    /// Starts <c>dart run &lt;scriptFileName&gt;</c> in <paramref name="workingDirectory"/> with the
    /// socket path and the authentication token in the environment.
    /// </summary>
    public DartScriptRun StartScript(string workingDirectory, string scriptFileName)
    {
        var startInfo = new ProcessStartInfo("dart")
        {
        ...
        return new DartScriptRun(startInfo, _outputHelper);
    }
...
/// <summary>
/// One running <c>apphost.dart</c>. The run stops at the ready marker until <see cref="Release"/>.
/// </summary>
internal sealed class DartScriptRun : IAsyncDisposable
{
    ...
    public DartScriptRun(ProcessStartInfo startInfo, ITestOutputHelper outputHelper)
    {
        _outputHelper = outputHelper;
    ...
    /// <summary>Returns whether the script reported <paramref name="key"/>.</summary>
    ...
    public async Task WaitForReadyAsync(TimeSpan timeout)
    {
        ...
        {
            throw new InvalidOperationException(
                $"The Dart script exited with code {await _exit} before it reported '{DartRoundTripHost.ReadyMarker}'.\n{Output}");
        }

        Kill();
        throw new TimeoutException(
            $"The Dart script did not report '{DartRoundTripHost.ReadyMarker}' in {timeout.TotalSeconds.ToString(CultureInfo.InvariantCulture)}s.\n{Output}");
    }
    ...
    public async Task<int> WaitForExitAsync(TimeSpan timeout)
    {
        ...
        if (completed != _exit)
        {
            Kill();
            throw new TimeoutException(
                $"The Dart script did not exit in {timeout.TotalSeconds.ToString(CultureInfo.InvariantCulture)}s.\n{Output}");
        }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        {
            _outputHelper.WriteLine("The Dart script did not exit after it was killed.");
        }
    ...
    private void OnStandardOutput(string? line)
    {
        ...
        if (string.Equals(line.Trim(), DartRoundTripHost.ReadyMarker, StringComparison.Ordinal))
        {
            _ready.TrySetResult();
            return;
        }

        if (line.StartsWith(DartRoundTripHost.ValuePrefix, StringComparison.Ordinal))
        {
            var pair = line[DartRoundTripHost.ValuePrefix.Length..];
