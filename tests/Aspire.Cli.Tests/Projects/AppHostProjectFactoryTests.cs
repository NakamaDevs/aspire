...
public class AppHostProjectFactoryTests(ITestOutputHelper outputHelper)
{
    ...
    [Fact]
    public void AppHostProjectFactory_DetectsDartAppHost()
    {
        using var workspace = TemporaryWorkspace.CreateForCli(outputHelper);
        var services = CliTestHelper.CreateServiceCollection(workspace, outputHelper, options =>
        {
            options.EnabledFeatures = [KnownFeatures.ExperimentalPolyglotDart];
        });

        using var provider = services.BuildServiceProvider();
        var factory = provider.GetRequiredService<IAppHostProjectFactory>();

        var appHostFile = new FileInfo(Path.Combine(workspace.WorkspaceRoot.FullName, "apphost.dart"));
        File.WriteAllText(appHostFile.FullName, "// test apphost");
    ...
    [Fact]
    public void AppHostProjectFactory_ReturnsNullForDartAppHost_WhenFeatureDisabled()
    {
        using var workspace = TemporaryWorkspace.CreateForCli(outputHelper);
        var services = CliTestHelper.CreateServiceCollection(workspace, outputHelper);
